...
public:
    ...
    friend class LinkedList; // Fungerr også som en forward-declaration
};
...
public:
    ...
    //Frivillig: Iterativ destruktør som forhindrer stack overflow
    // ~LinkedList() {
    //     auto node = end()->prev;       // Starter på siste element
    //     while(node->prev != nullptr) { // Itererer til starten av lista
    //         node->next = nullptr;      // Kobler av siste element
    //         node = node->prev;         // Hopper til noden tidligere i lista
    //     }
    // }
